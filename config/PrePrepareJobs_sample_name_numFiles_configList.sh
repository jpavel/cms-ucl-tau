...
more ${config_name} | sort > temp_input.1
...
do
  ...
  echo "source PrepareJobs.sh ${input_data} ${output_name}_${counter} ${num_files} ${file}" >> Prepare_${output_name}.sh
  echo "echo \"./SubmitAll_${output_name}_${counter}.sh\" >> Run_${output_name}.sh" >> Prepare_${output_name}.sh
done
