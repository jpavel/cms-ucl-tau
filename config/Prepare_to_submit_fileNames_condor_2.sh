...
do
  ...
  mkdir ${1}/job${counter}
  cp JobConfig.dtd ${2} Fall11_PU.root Fall11_PU_observed.root Summer12_PU.root Summer12_PU_53X.root dataPileUpHistogramABCD_True_2012.root dataPileUpHistogram_Observed_2011.root dataPileUpHistogram_True_2011.root ${1}/job${counter}
  cp ${1}/${file} ${1}/job${counter}/input.xml
  echo "Initialdir     = ${1}/job${counter}" >> ${1}.cmd
  echo "Arguments      = ${2}" >> ${1}.cmd
  echo "queue 1" >> ${1}.cmd
  echo "" >> ${1}.cmd
done
