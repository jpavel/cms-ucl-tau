...
int MakeTable(TString inputFile = "/home/jpavel/ntuples/CMS/analysis/ZHtautau/NewCuts/53x/MC/NewCuts_Htt125_histo.root")
{
	 gROOT->Reset();             
  //SetAtlasStyle();
  setTDRStyle();
  ...
  f = TFile::Open(inputFile);
	...
	TH1D* h_event_type_raw = (TH1D*)f->Get("h_event_type_raw");
	...
	for(uint i =1; i < 9; i++)
	{
		std::cout <<  h_event_type->GetXaxis()->GetBinLabel(i) << " : " << h_event_type->GetBinContent(i)*weight << ": " << h_event_type_raw->GetBinContent(i) << std::endl;
	...
	ofstream tab1;
	tab1.open("Htt125_8TeV_yield.tab");
