...
int estimateBG(TString inputDir = "/home/jpavel/analysis/CMS/histograms/PostMoriod/Application2/") {
  ...
  TString outputDir = "/home/jpavel/analysis/CMS/Plots/BGestimate_check";
  ...
  const uint nHist2 = 3;
  TString histNames2[nHist2] = {"h_jet_category0_jet_pt_", 
	"h_jet_category1_jet_pt_", "h_jet_category2_jet_pt_"
	};	
 ...
 std::cout << h_hist[0][0][0]->GetSumOfWeights() << std::endl;
   ...
   TF1 *ele_FR_loose = new TF1("eleFit","[0]+[1]*TMath::Exp([2]*x)",10.,90.);
   ele_FR_loose->SetParameters(1.17877e-02,1.06119e+00,-1.25356e-01);
   TF1 *ele_FR_medium = new TF1("eleFitM","[0]+[1]*TMath::Exp([2]*x)",10.,90.);
   ele_FR_medium->SetParameters(4.19025e-03,3.03030e-01,-1.07820e-01);
   
   //~ const double FR_mu_loose=0.0379521;
   //~ const double FR_mu_tight=0.035781;
   TF1 *mu_FR_loose = new TF1("muFit","[0]+[1]*TMath::Exp([2]*x)",10.,90.);
   mu_FR_loose->SetParameters(2.76045e-03,7.09585e-01,-7.43405e-02);
   TF1 *mu_FR_medium = new TF1("muFitM","[0]+[1]*TMath::Exp([2]*x)",10.,90.);
   mu_FR_medium->SetParameters(5.07951e-03,6.80099e-01,-7.68151e-02);
   
   TF1 *FR_medium = new TF1("fit1","[0]+[1]*TMath::Exp([2]*x)",10.,90.);
   FR_medium->SetParameters(9.86910e-03,2.18583e+00,-1.04698e-01);
		   ...
		   for(int jBin=1; jBin <= h_hist2D[0][0][iType]->GetNbinsY()+1;jBin++)
		   {
			...
			switch(iType%4)
			{
				case 0:
					...
					fr2=mu_FR_medium->Eval(pt2);
					break;
				case 1:
					fr1=ele_FR_loose->Eval(pt1);
					fr2=mu_FR_loose->Eval(pt2);
					break;
				case 2:
					fr1=FR_medium->Eval(pt1);
   ...
   for(uint iType=0; iType < nTypes; iType++){
	   ...
	   for(int iBin = 1; iBin <= h_hist2[0][1][iType]->GetNbinsX()+1; iBin++)
	   {
		   ...
		   switch(iType%4)
			{
				case 0:
					fr1=mu_FR_medium->Eval(pt1);
					break;
				case 1:
					fr1=mu_FR_loose->Eval(pt1);
					break;
				case 2:
  ...
  ofstream tab1;
	tab1.open(outputDir+"/bgestimate_jet.tab");
