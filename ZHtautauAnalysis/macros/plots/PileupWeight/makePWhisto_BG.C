...
int makePWhisto(TString filename_MC="/home/jpavel/ntuples/CMS/analysis/ZHtautau/Lucia/histo_42X_TruePU_v2/DY_42X_TruePU.root",TString filename_data="/home/jpavel/analysis/CMS/ZHtautau/ZHtautauAnalysis/config/dataPileUpHistogram_True_2011.root")
{
	gROOT->Reset();             
   SetAtlasStyle();
   gStyle->SetPalette(1);
	...
	float Max_h = *max_element(myMax,myMax+3);
	...
	TLegend* leg = new TLegend(0.55,0.6,0.85,0.93);
	leg->SetBorderSize(0);
	leg->SetFillColor(0);
	leg->SetTextSize(legend_size);
	leg->AddEntry(mu_RW_import,"re-weighted MC","p");
	leg->AddEntry(mu_data,"data 2011","lp");
	leg->AddEntry(mu_orig_import,"Fall11 MC","l");
	
	leg->Draw();
	
	c1->Print("mu_reweight_BG.png");
	c1->Print("mu_reweight_BG.eps");
