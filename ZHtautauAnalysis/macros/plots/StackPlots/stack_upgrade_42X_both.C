...
int stack_upgrade_42X() {
  ...
  //DoubleEle + WZjets 
  TString fileNames[nFiles] = {"Data","ZZ_42X_TruePU","WZJets_42X_TruePU","TT_42X_TruePU","DY_42X_TruePU","Htt120_42X_TruePU","Hww120_42X_TruePU"};
  //DoubleMu + WZ 
 ...
 //DoubleMu + WZ 
  ...
  for(int iFile = 0; iFile < nFiles; iFile++)
  {
	  for(int iHist = 0; iHist < nHist1; iHist++)
	  {
		  ...
		  if(iHist == 1 || iHist == 3 || iHist==4) h_1d[iHist][iFile]->Rebin(10);
	  }
  }
...
TString lumist="4.9 fb^{-1}";
  ...
  ll->AddText(0.5, 0.5, text);
	...
	// if(iHist > 0 && iHist < 4)  h_1d[iHist][0]->GetXaxis()->SetRangeUser(0,150);
	  ...
	  c1->SetLogy();	
	  c1->Print("NewCuts/Both_"+histNames1[iHist]+"_all.png");
	  c1->Print("NewCuts/Both_"+histNames1[iHist]+"_all.eps");
	  c1->SetLogy(0);
	  h_1d[iHist][0]->GetYaxis()->SetRangeUser(0,1.5*max);
	
	  c1->Print("NewCuts/Both_"+histNames1[iHist]+"_all_normal.png");
	  c1->Print("NewCuts/Both_"+histNames1[iHist]+"_all_normal.eps");
