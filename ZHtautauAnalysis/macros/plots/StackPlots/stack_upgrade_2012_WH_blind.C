...
int stack_upgrade_WH() {
  ...
  //DoubleEle + WZjets 
  TString fileNames[nFiles] = { "WZJets"
  };
  
  TString inputDir = "/home/jpavel/analysis/CMS/histograms/WH/20131014/Summary/";
  
  TString outputDir = "/home/jpavel/analysis/CMS/Plots/Stack/WH/20131014/WZJetsBlind";
  ...
  TString PaveText[nHist1]	= {"CMS preliminary 2012 LT > 130 GeV", "CMS preliminary 2012 LT < 130 GeV", "CMS preliminary" };//,"MMMT","MMME","MMET","EETT","EEMT","EEME","EEET","EETT"};
 ...
 // TH1F *                signal[nHist1];
  ...
  Double_t xsection[nFiles]={1.057};
  //{1.0, 1.057, 1.057, 0.130};//, 0.208, 0.01203, 0.0048}; // pb
  Double_t events[nFiles]={2017979};
//  {1.0, 2979624, 2017979, 4989540};//, 210160, 400973, 548760}; // pb
  
  Color_t colors[nFiles]={kBlue};
  //{kBlack,kRed,kRed,kBlue};
...
//~ 
  ...
  for(int iHist = 0; iHist < nHist1; iHist++) 
  {
	  ...
	  for(int iFile=0; iFile < nFiles; iFile++)
	  {
		  ...
		  //if(iFile < 5) 
	//	 if(iFile!=1) hs->Add(h_1d[iHist][iFile],"hist");		
...
//~ 
 ...
 leg->SetBorderSize(0);
	...
	//  leg->AddEntry(h_1d[iHist][1],"WZ","f");
	  leg->AddEntry(h_1d[iHist][0],"WZ","f");
...
//~ 
  ...
  //    h_1d[iHist][0]->Draw("samePE01");
	  ...
	  leg->Draw("same");
	  ll->Draw("same");
	  if(iHist==2) { c1->SetLogy();
					h_1d[iHist][0]->GetYaxis()->SetRangeUser(5,5e8); }
	  gPad->RedrawAxis();
//~ 
	  //~ c1->SetLogy();	
