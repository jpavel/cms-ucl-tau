...
namespace SVfit_namespace
{
  ...
  TLorentzVector boostToCOM(const TLorentzVector&, const TLorentzVector&);

  /// Boost a lorentz vector given in the rest frame of another lorentz vector to the laboratory frame.
  TLorentzVector boostToLab(const TLorentzVector&, const TLorentzVector&);
  ...
  TVector3 motherDirection(const TVector3&, double, double);

  /// Compute the tau four vector given the tau direction and momentum
  TLorentzVector motherP4(const TVector3&, double, double);

  /// Compute decay angles in rest frame given momentum of tau lepton and visible decay product in lab frame
  double gjAngleFromLabMomenta(const TLorentzVector&, const TLorentzVector&);
  TVector3 normalize(const TVector3&);
  double compScalarProduct(const TVector3&, const TVector3&);
  TVector3 compCrossProduct(const TVector3&, const TVector3&);
  double phiLabFromLabMomenta(const TLorentzVector&, const TLorentzVector&);
  
  void printVector(const std::string&, const TVector3&);
  void printMatrix(const std::string&, const TMatrixD&);
