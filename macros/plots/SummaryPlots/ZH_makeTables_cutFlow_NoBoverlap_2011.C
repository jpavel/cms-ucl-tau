...
int MakeTable(bool useData = true)
{
  ...
  for(int iFile = 0; iFile < nFiles; iFile++)
  {
   ...
   TString BinLabels2[4] = { "H selected", "Overlap cleaning", "Same vx", "B-tag veto"};
	...
	ofstream tab1;
	tab1.open("/home/jpavel/Documents/UCL/HiggsTauTau/ZH/cutflow/2011_cutFlow_NoB_data.tab");
		tab1 << "\\begin{sidewaystable}[h]" << std::endl;
	tab1 << "	\\begin{center}" << std::endl;
	tab1 << "		\\caption{event yield 2011 (no b-tau overlap) (4.9 fb$^{-1}$)}\\label{tab:CutFlow_NoB}" << std::endl;
