...
int MakeTable(TString inputFile = "Htt125")
{
  ...
  setTDRStyle();
  gStyle->SetPalette(1);
  TString inputDir="/home/jpavel/ntuples/CMS/analysis/ZHtautau/CutOpti/CutOpti_";
  ...
  {
	indexes.str("");
  	indexes << inputDir << inputFile << "_cut" << iFile << "_histo.root";
	...
	std::cout << "		\\begin{tabular}{l|rcl|rcl|rcl|rcl|rcl|rcl|} \\hline" << std::endl;
	std::cout << setiosflags(ios::fixed) << std::setprecision(2) << "		Event Type & cut0 &  & & cut1 &  & & cut2 &  & & cut3 &  & & cut4 &  & & cut5 &  & \\\\ \\hline" << std::endl;
	for(uint i = 1; i < 9; i++){
	...
	tab1 << "		\\begin{tabular}{l|r|r|r|r|r|r} \\hline" << std::endl;
	tab1 << setiosflags(ios::fixed) << std::setprecision(2) << "		Event Type & cut0 & cut1 & cut2 & cut3 &cut4 & cut5  \\\\ \\hline" << std::endl;
	for(uint i = 1; i < 9; i++){
		tab1 << BinLabels[i-1];
		for(uint j =0; j < nFiles; j++){
