...
//--- get parameters defining number of "stochastic moves" performed per integration
  numIterBurnin_ = 10000;
  numIterSampling_ = 100000;
...
MarkovChainIntegrator::MarkovChainIntegrator(std::string name="", std::string mode="Metropolis", std::string initMode="none", unsigned numIterBurnin =10000,
unsigned numIterSampling=100000, unsigned maxCallsStartingPos=1000000, unsigned numIterSimAnnealingPhase1=2000, unsigned numIterSimAnnealingPhase2=2000,
double T0=15., double alpha=0.999, unsigned numChains= 1, unsigned numBatches=1, unsigned L=1, double epsilon0=1.e-2, bool useVarEpsilon0 = false,
double nu = 0.71, int verbosity=0 )
  : name_(""),
    integrand_(0),
    startPosition_and_MomentumFinder_(0),
    x_(0),
    ...
    monitorTree_(0)
{
  ...
  std::string moveMode_string = mode;
  ...
  std::string initMode_string = initMode;
...
//--- get parameters specific to "dynamic moves" 
  L_ = L;
    epsilon0_  = epsilon0;
    useVariableEpsilon0_ = useVarEpsilon0;
