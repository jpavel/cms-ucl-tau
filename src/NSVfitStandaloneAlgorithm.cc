...
NSVfitStandaloneAlgorithm::integrateMarkovChain()
{
  ...
  if(isInitialized2_){
    ...
    double nu = 0.71;
    std::string name = "NSVfitStandaloneAlgorithm";
    int verbosity = -1);
    //cfg.addParameter<int>("verbosity", 2);
    integrator2_ = new MarkovChainIntegrator("", mode, initMode, numIterBurnin, numIterSampling, 1000000,  numIterSimAnnealingPhase1, numIterSimAnnealingPhase2, 
    T0, alpha, numChains, numBatches, L, epsilon0, false ,nu , verbosity);
